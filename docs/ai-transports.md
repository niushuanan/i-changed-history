...
| --- | --- | --- | --- | --- |
| 抖音互动空间审核包 | `tt.callAIChatCompletion`，由平台转发至火山方舟；每次调用固定 `reasoning_effort: "minimal"` | 抖音平台 AI 服务配置 | `doubao-seed-2-0-lite-260428` → `doubao-seed-2-0-pro-260215` → `doubao-seed-evolving` | 第一幕固定；其余调用全部不思考，只有明确额度耗尽才切下一模型 |
...
两条通道都一次生成六张决定牌：首组三张立即展示，第二组三张供本节点第一次 Roll 使用。第二、第三次 Roll 会分别发起一次新的 A/B/C 三牌请求。互动空间的正常生成、字段修复、全局恢复、Roll 和双结局全部由 `src/services/seed.interactive.ts` 强制压到 Seed 的 `minimal` 模式；上层即使为其他模型请求高推理，也不能在该包中重新打开思考。
...
- `src/services/seed.interactive.test.ts`：互动空间环境验证 `tt.callAIChatCompletion`、官方原始文本 SSE、空流的同模型非流式接管、旧版 provider SSE、平台信息事件、五位业务错误码、Lite → Pro → Evolving 的额度耗尽接力、30 分钟会话冷却、普通限流不切模型，以及三种 Seed 全部强制 `reasoning_effort: "minimal"`。
