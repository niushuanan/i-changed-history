...
| --- | --- | --- | --- | --- |
| 抖音互动空间审核包 | `tt.callAIChatCompletion`，由平台转发至火山方舟；每次调用固定 `reasoning_effort: "minimal"` | 抖音平台 AI 服务配置 | `doubao-seed-2-0-lite-260428` | 第一幕固定；其余调用全部不思考并优先低延迟 |
...
- `src/services/seed.interactive.test.ts`：互动空间环境验证 `tt.callAIChatCompletion`、官方原始文本 SSE、空流的非流式接管、旧版 provider SSE、平台信息事件、五位业务错误码，以及固定 Doubao Seed 2.0 Lite 和强制 `reasoning_effort: "minimal"`。
