...
- A refresh or development-server reconnect during generation must resume the saved AI request automatically. Structural drift receives contextual repair and regeneration; if all model attempts fail, preserve the run and show an explicit retry instead of inventing a generic local historical scene.
...
- Turn and ending calls use DeepSeek high-reasoning mode, an 8192-token ceiling, and a 90-second transport window. There is exactly one turn request per node and one ending request per run.
...
- Event pages use one vertical height system at 390 x 844: the progress header occupies normal flow above the scene image, never overlays it, and the three actions expand to fill the remaining viewport without a blank lower half.
