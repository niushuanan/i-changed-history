...
- The profile controls cross-generation advantages and their costs: exactly one generated action per node visibly uses the traveler's modern strengths. During play the user may also spend one of exactly three per-run `第四条路` opportunities to submit a free action; DeepSeek must adjudicate it against the current identity, resources, technology, information, and deadline before it becomes history.
...
- The four-letter time personality is a gameplay contract, not decoration: exactly one choice per node follows it, its causal-preview lens changes what consequence the player can reveal, and it changes how DeepSeek adjudicates all three free-history actions.
...
- A free fourth-path request spends its opportunity only after a valid ruling is committed. Network failure, model repair, refresh recovery, and retry never consume an opportunity; successful custom actions enter the same causal memory and 2026 ending contract as generated choices.
