...
- The profile controls cross-generation advantages and their costs: exactly one generated action per node visibly uses the traveler's modern strengths. Each of the four personality dilemmas has exactly four distinct choices so the result feels chosen rather than binary-tested.
...
- The AI is framed as a grounded world simulator. It generates the three ordinary actions against historical constraints, but it never vetoes a direct player rewrite. For direct rewrites, the client owns the immediate receipt and the AI owns only the next-node world extrapolation. The visible proof is source provenance, a causal receipt, and a new era-appropriate identity, not long model prose.
...
- From chapter 2 onward, a deterministic client-side ripple router selects a new social carrier and excludes the previous two carriers. DeepSeek may keep the story in the same country, but it must show how the prior result enters a different institution, population, or domain through a concrete causal bridge.
