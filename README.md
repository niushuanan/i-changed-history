...
《哎！我改变了历史？》是一款面向移动端的 AI 架空历史游戏。它不是历史知识问答，也不是让玩家旁观一段随机续写。玩家从 13 个按时代组织的剧本组中选定一组，再成为其中某个真实转折点里的具体参与者，在同一个人的一生里做出四次决定，看一个选择怎样改变下一场冲突、一个时代，以及自己死后直到 2026 年的世界。

## 游戏的核心体验

- **从熟悉的真实历史开始**：100 个真实转折点按时间排列，其中 58 个来自中国史、42 个来自中国玩家熟悉的世界史，覆盖公元前到现代。
- **沿时代逐组深入**：100 个剧本被完整分进 13 组——9 组中国史、4 组世界史。首页只展示组别、时代范围和进度，进入已解锁组后再选择具体历史，不把一百张卡同时压给玩家。
- **通关换下一把钥匙**：任意第一组免费解锁；此后每解锁一组消耗 1 枚代币。某个剧本的两份结局报告生成后，首次通关奖励 1 枚代币；重复游玩保留档案，但不会重复发币。
...
1. 首页展示 13 个剧本组。新玩家任选第一组免费解锁；之后使用通关所得代币逐组开启。
2. 进入已解锁组后选择具体历史，便立即进入该剧本的固定第一幕，不需要等待模型；已经通关的剧本可以重复游玩。
...
5. 第 4 次决定后，游戏并发生成自然普通话“一生纪事”，以及主角死后延伸到 2026 年的小说式世界报告；两份报告都完成后，首次通关会永久点亮该剧本并奖励 1 枚解锁代币。

## AI 如何参与

| 环节 | 实现方式 |
| --- | --- |
| 13 个剧本组、100 张历史卡与第一幕 | 本地固定数据，选定后立即可玩 |
| 第 2-4 幕 | 当前运行环境的实时模型生成；互动空间按 Lite → Pro → Evolving 额度接力，三者都固定为 `high` 最高思考强度 |
...
| 最终人物列传 | 当前运行环境的模型独立生成 |
| 2026 平行世界报告 | 与人物列传并发生成 |

第一幕固定，是为了让玩家选中历史后立刻开始；每个历史快照预先拥有六张 AI 写成的天外候选，运行时随机取两张而不增加网络等待。真正需要开放推理的第 2 至第 4 幕、第二与第三次 Roll、以及最终报告由模型生成。实时 Roll 只换牌、不改写当前现场，失败时保留原牌、同一能力和次数并可直接重试。所有模型结果都使用 JSON Schema 风格契约和 Zod 校验。轻微字段错误只修复失败字段；无法修复时保留游戏进度并提供重试，不在本地伪造后续剧情。
...
- Zod 结构校验与字段级 AI 修复
- reducer 驱动的可恢复游戏状态机
- localStorage 当前局存档与独立的分组解锁进度、Web Audio 配乐、完整滚动尺寸的 2x PNG 结局导出
...
- `src/data/historyGroups.ts`：13 个剧本组及其 100 个历史节点的唯一归属
- `src/screens/SeedPickerScreen.tsx`：分组档案墙、组内剧本选择、锁定预览、完成进度与代币反馈
- `src/hooks/useUnlockProgress.ts` 与 `src/services/unlockProgress.ts`：版本化分组解锁、首次通关和旧档案迁移
...
- `src/services/seed.interactive.ts`：抖音互动空间 `tt.callAIChatCompletion` / 火山平台传输，按 `doubao-seed-2-0-lite-260428` → `doubao-seed-2-0-pro-260215` → `doubao-seed-evolving` 接力，统一使用 `reasoning_effort: "high"` 与 `service_tier: "auto"`
...
该命令会把完整 100 个历史开局、固定首幕、AI 续幕、音频和本地图片一起转换为互动空间运行时，通过平台 `tt.callAIChatCompletion` 优先调用 Doubao Seed 2.0 Lite；只有平台明确返回“额度耗尽”时，才依次切到 Seed 2.0 Pro 和 Seed Evolving。三种模型的续幕、Roll、修复、恢复和结局请求全部锁定为 `reasoning_effort: "high"` 的最高思考强度，并携带 `service_tier: "auto"`，在账户已开通 Fast 时优先使用低延迟服务、否则自动回退普通服务层；普通限流、网络波动和服务错误仍在原模型有限重试，不会误切模型。构建随后优化全部移动端资源、检查 100 个剧本和图片均已入包、拒绝任何 DeepSeek 运行时痕迹、确认 ZIP 不超过 8MB，并生成 `release/i-changed-history-interactive-space.zip`。不要手工压缩普通网页构建并提交审核。
