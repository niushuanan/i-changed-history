...
- **让命运替你做第一道选择**：按下抽取后，一百节点时间线会疾驰并停在唯一一个现场；优先抽中还没有通关解锁的历史，避免面对一百张卡片纠结。
- **通关才能点亮档案**：完整走完一个历史开局的四次抉择后，该节点永久进入“已解锁档案”；没有完成的节点仍保持未知。
- **活完一个人的一生**：主角在四个节点中保持同一个名字和身体，从命运当日、三日余波走到人生转折和最后抉择。
- **像肉鸽一样出牌**：每一幕只出现三张牌——循史、破局、天外。向上划出一张就把它写入时间线，不提供自由输入，也没有隐藏的第四选项。
- **每幕可以 Roll 三次**：第一次立即翻出已经准备好的第二组三张；第二、第三次由 AI 针对当前现场实时发出新牌，并避开已经看过的手段和奇想。
...
1. 首页先显示一张未显影的命运牌。按下抽取，时间线快速滚动并随机停在一个历史节点。
2. 揭晓后仍使用完整历史档案卡；确认闯入便立即进入固定第一幕，不需要等待模型。
3. 每幕从循史、破局、天外三张牌中选择一张；本幕可以 Roll 三次，第一次秒开，后两次实时生成。
...
第一幕固定，是为了让玩家抽中历史后立刻开始；真正需要开放推理的第 2 至第 4 幕、第二与第三次 Roll、以及最终报告由模型生成。实时 Roll 只换牌、不改写当前现场，失败时保留原牌并可直接重试。所有模型结果都使用 JSON Schema 风格契约和 Zod 校验。轻微字段错误只修复失败字段；无法修复时保留游戏进度并提供重试，不在本地伪造后续剧情。
...
- `src/screens/SeedPickerScreen.tsx`：随机命运抽取、百节点疾驰时间线与已解锁档案
- `src/components/GameAnnouncement.tsx`：首次进入与设置菜单可重看的玩家玩法公告
