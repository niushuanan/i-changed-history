...
- `src/services/`：中性 `completion.ts` / `completion-contract.ts` 模型边界、DeepSeek 官方 / Sites Worker 传输、带 Lite → Pro → Evolving 额度接力的独立 Seed 互动空间传输、带短时配乐 ducking 的前景卡牌音效、版本化当前局存储、独立永久解锁收藏、低响度史诗配乐，以及完整报告 PNG 的准备、系统分享、下载与资源回收。
...
### 2026-07-30 13:15 - 同步首次 Roll 牌组去重提示

- 本次任务：把 `main` 已完成的首次 Roll 牌组去重提示同步到 `codex/seed-lite-minimal`。
- 改了哪些文件：修改 `src/game/prompts.ts` 和本文档。
- 改了什么：把续幕输出示例中 `r` 的说明从“与 c 相同”改为“与 c 结构相同、内容不得重复的三项新牌数组”，明确只复用数组结构，不能复用首发三张牌的内容。
- 为什么这样改：旧文案可能让 Seed 把预生成 Roll 牌组直接复制成首发牌组，导致玩家第一次 Roll 后看见原牌。
- 影响了哪些模块：只影响 Seed 分支续幕一次生成中的预生成第一次 Roll 牌组提示；不改变 Schema、三次 Roll 流程、实时第二/第三次 Roll、最高思考、Fast 服务层、三模型接力、交互或发布配置。复核后第 1–3 节仍准确，无需调整。
