...
《哎！我改变了历史？》是一款移动端 AI 穿越历史肉鸽卡牌游戏。100 个著名真实历史转折点（中国 58、世界 42，覆盖公元前到现代）被完整且唯一地分进 13 个剧本组：9 组中国史、4 组世界史。首页是延续煤黑、新闻纸、朱砂、青绿与黄铜质感的分组档案墙，只展示组别、时代、范围、锁定状态和完成进度；进入已解锁组后再选择具体历史，避免把一百张卡同时压给玩家。新玩家可任选第一组免费解锁，此后每解锁一组消耗 1 枚代币。某个剧本的《一生纪事》和 2026 蝴蝶效应报告都生成并进入结果页后，首次通关永久记录该剧本并奖励 1 枚代币；重复游玩不会重复发币。分组、通关剧本与代币使用独立的版本化 localStorage 信封，损坏值会被清洗，旧版已解锁档案会迁移并保留原有访问权。

玩家选中具体历史后立即装载该节点固定且经过 schema 校验的第一幕。每幕先展示循史、破局、天外三张牌并允许 Roll 三次：第一次经过约 1.2 秒实体洗牌仪式换成预生成第二组，第二和第三次在保持现场不变的前提下由当前运行环境的 AI 实时生成新三张。循史让当前掌权者、命令方向与既有结果按时落地且不新增偏离度；破局和天外才改变控制关系或现实规则。100 个固定第一幕的两组 A/B 都在开发期由 AI 编写并审校，天外牌由客户端从 50 项超能力中整局无放回抽取。牌面保持短标题，长按显示完整决定、行动规格、结果与代价，向上划出后把完整决定写入同一主角的一生；产品不提供自由输入、第四路径、人格测试、MBTI 标签或隐藏时间线。

当前版本保留完整的浏览器端游戏，同时已经具备可公开分享的 Sites / Cloudflare Worker 托管架构：浏览器只向同源 `/api/deepseek/completions` 发送固定游戏协议，Worker 从运行时 secret 读取 DeepSeek `deepseek-v4-flash` 密钥，并通过官方 `/v1/chat/completions` 原样流式转发 SSE；浏览器产物不再包含 API Key。localhost 和 Sites 都不设置产品侧访客、IP、全站、分钟或每日请求限额，也不再创建 D1 用量桶；通过版本、同源、系统协议、阶段、请求类型和 JSON 结构校验的游戏请求会立即转发。Worker 不再检查具体中文 `task` 文案，因此 prompt 演进不会再次触发产品侧 400；DeepSeek 上游仍可能按供应商容量或账户状态返回 429。互动空间完全绕过该 Worker，由中性的 `src/services/completion.ts` 入口在构建时切换到 `src/services/seed.interactive.ts`，只通过 `tt.callAIChatCompletion` 使用平台托管的火山凭据，并严格按 `doubao-seed-2-0-lite-260428` → `doubao-seed-2-0-pro-260215` → `doubao-seed-evolving` 接力；只有平台明确返回额度或配额耗尽时才进入下一模型，普通 429、网络波动、5xx、鉴权、未开通、参数错误和空流都不会误触发切换。已耗尽模型在当前标签页记忆 30 分钟，后续调用直接跳过；正常续幕、Roll、字段修复、全局恢复、空流接管与双结局在三个模型上都强制发送 `reasoning_effort: "high"` 与 `service_tier: "auto"`，上层的低推理请求也不能关闭 Seed 的最高思考强度，账户可用时优先走 Fast、不可用时自动走普通服务。互动空间按平台最新 API 定义直接拼接 `onSSE(event.data)` 文本分片，同时兼容旧基础库透传的 OpenAI `choices[].delta` 包。健康流已有正文时忽略可能重复的 `success.data`；若平台结束空流，则在同一模型上通过同一个官方 API 自动以 `stream=false` 重试一次并读取完整 `success.data`，该接管请求仍保持 `high` 与 `auto`。平台五位业务错误码不会再被误当成 HTTP 5xx，错误类型和错误码会保留在玩家可见的重试信息中。本地与 Node 长测优先读取项目 `.env.local` 的当前 Key，避免失效的系统环境 Key 覆盖用户刚配置的凭据；发布长测必须通过本地 Worker 代理运行，直连官方接口的 Node 长测只用于拆分上游故障，不能证明浏览器产品可用。后续幕次使用九位场景数组加两组三牌数组的低延迟协议，只传对连续性、校验或可见故事有真实消费方的上下文，完整中文决定与结果仍由模型编写；常规幕次使用 4096 输出 token，并发双结局各使用 2048，本地/Sites 走 V4 Flash 非思考快速路径，互动空间走三种 Seed 的 `high` 最高思考路径并优先 Fast；其中 `doubao-seed-evolving` 是无版本号的持续更新 Model ID，不需要随版本发布改代码。十秒只作为真实长测中的优化目标，不是运行时截止线，超过十秒的健康请求会继续流式生成，完整文案、字段修复和剧情连续性优先。Zod 继续校验结构化输出，也会在本地补齐无语义变化的句末标点、接纳完整的自然长度，并在模型多写身后时代时保留最早三段与最终 2026 段；人物结局中的死亡年份和年龄由客户端权威渲染，模型只提供纯地点，若新响应或旧存档在地点前重复年龄，schema 会在显示前去重。前端公式继续计算历史偏离度，本地 CC0 配乐以低响度随章节缓慢变化，八项交互音效作为前景反馈并在播放时短暂压低配乐；两页结局仍按完整滚动尺寸导出为 2x PNG，桌面端直接下载，移动 Web 通过第二次用户操作打开系统分享面板并保留 PNG 下载后备。当前局存档与分组解锁进度分别保存在浏览器 `localStorage`：局内状态可按版本淘汰，已解锁组、首次通关剧本与未使用代币会独立恢复。
...
项目还保留一个与正式游戏完全隔离的互动空间 AI 最小诊断页 `diagnostics/interactive-ai-mvp/`。它只使用官方最小非流式调用：`type=text`、精确 Model ID `doubao-seed-2-0-lite-260428`、`reasoning_effort=high`、`service_tier=auto`、一条 `user` 消息和 `stream=false`，不带 system prompt、温度、复杂 JSON、SSE、自动重试或游戏 schema，并把 `success`、`fail`、`complete` 原始回调直接显示在手机页面。`npm run build:interactive-ai-mvp` 只生成被忽略的 `release/interactive-ai-mvp.zip` 诊断包，不进入、替换或改变正式 `npm run build:interactive` 产品包。平台独立草稿 AppID `tt18806e3d87aab3e421` 只服务扫码诊断，不提交产品审核。

项目只维护 `main` 上这一份完整 100 剧本源码。抖音互动空间包由 `npm run build:interactive` 从同一份代码即时生成：AI 传输在构建时替换为真实 `tt.callAIChatCompletion`，火山方舟 Model ID 严格按 Lite → Pro → Evolving 的额度耗尽顺序接力，三个模型的 `reasoning_effort` 都固定为 `high`，每次请求同时发送 `service_tier: "auto"`，密钥只使用互动空间账号托管的火山引擎 API Key；全部 100 个历史开局、固定首幕和对应本地历史图均进入提交 ZIP。脚本检查完整剧本和图片数量、三个真实 Seed Model ID 与接力规则、最高思考和 Fast 自动服务层、清理 macOS 元数据、优化全部移动资源、拒绝任何 DeepSeek 运行时字样、执行 8MB ZIP 闸门并生成待上传文件；首页起始画面标题栏角落保留清晰的双行 `AI / 生成` 合规标识，20px 字高不低于 390px 产品画面短边的 5%，事件现场和两页报告不再重复展示固定开场、AI 来源或模型版本，互动空间用 `体验说明` 避免平台禁用词。`interactive-space.release.json` 把当前发布目标 AppID、竖屏、ZIP、AI 开启、火山平台凭据、三个真实模型 ID、额度耗尽接力、30 分钟冷却、`high` 最高思考强度、`auto` Fast 优先、抖音 39.5.0 最低测试版本和显式标识下限写成机器可查的发布契约，`20107` 会阻断提审。当前发布目标是 AppID `tt68891e34f7a3c3e821`；它保持草稿状态，供每次覆盖后扫码实测再决定是否提审。上一独立作品 `ttc88abe3757a063ed21` 保留其既有审核版本，原 AppID `tt8b1afda71430200121` 也未被更新、撤审或删除。

## 2. 代码结构是什么

- `src/data/`：100 张著名历史转折点、与其一一对应的固定第一幕、13 个剧本组，以及按年份/阶段选取的视觉资产映射。
- `src/data/historySeeds/`：历史剧本模块边界；`scripts/<seed-id>/index.ts` 每个目录只拥有一个剧本，`shared.ts` 提供公共构造器与标签，`index.ts` 显式维护完整 100 节点的唯一聚合顺序。
- `src/data/historyGroups.ts`：9 个中国史组、4 个世界史组和全部 100 个 seed ID 的唯一归属、顺序与查询入口。
...
- `src/hooks/`：`useGame.ts` 负责当前局的请求、Roll、存储、音频和重试编排；`useUnlockProgress.ts` 独立负责组解锁、首次通关和代币余额。
- `src/services/`：中性 `completion.ts` / `completion-contract.ts` 模型边界、DeepSeek 官方 / Sites Worker 传输、带 Lite → Pro → Evolving 额度接力的独立 Seed 互动空间传输、带短时配乐 ducking 的前景卡牌音效、版本化当前局存储、独立 `unlockProgress.ts` 分组进度信封、低响度史诗配乐，以及完整报告 PNG 的准备、系统分享、下载与资源回收。
...
- `src/screens/` 和 `src/components/`：从分组档案墙、锁定预览、组内选剧本、游戏说明、三卡牌桌、三次 Roll、长按详情、上划提交，到同一主角死亡、首次通关奖励与 2026 身后历史报告的完整界面。
...
- `interactive-space.release.json` 与 `docs/interactive-space-release-checklist.md`：互动空间 AppID、AI 开关、Lite → Pro → Evolving 三个真实 Model ID、仅额度耗尽接力、30 分钟冷却、`high` 最高思考强度、`service_tier: "auto"` Fast 优先、平台火山凭据、抖音 39.5.0 最低测试版本、起始画面 5% AI 显式标识、竖屏、ZIP 上限、`20107` 阻断与提审前真机门禁。
...
实际数据流是：读取版本化分组进度 -> 展示 13 组档案墙和完成数 -> 玩家免费解锁第一组，或花费 1 枚代币解锁后续组 -> 进入组内并选择具体历史 -> 客户端读取该历史已经由 AI 生成并审校的两组 A/B，再从该历史专属六张 C 候选中随机取两项能力，同时打乱剩余 48 项能力并把整个牌堆写进当前局存档 -> 同步装载固定第一幕、固定主角与两组三张牌 -> 玩家可直接选首组，也可第一次 Roll 在约 1.2 秒实体洗牌后无网络地换成预生成第二组；第二、第三次 Roll 先无放回取一项能力，再把它、当前现场和所有已看牌提交给模型，只接收新的 A/B/C 三张，并在请求成功或失败前保持可见洗牌过渡，失败重试继续使用同一个 `powerId` -> 长按查看完整字段，向上划出一张 -> reducer 把完整 `label`、能力 ID、偏离类型与即时结果固化为不可撤销 `WorldCanon`；A 不新增偏离度，B/C 才会继续推高改变 -> 结果页为下一幕预取两项不同能力 -> 第 2 至第 4 幕把全部已选牌压成分层叙事上下文，本地/Sites 通过官方 `/v1/chat/completions` 发起 SSE 推演，互动空间只由 `tt.callAIChatCompletion` 使用平台托管的火山凭据和三个 Seed 模型的 `high` 最高思考参数及 `service_tier: "auto"`，明确额度耗尽时才从 Lite 向 Pro、Evolving 接力 -> 本地/Sites DeepSeek 传输完成或失败时同步尝试向 `llm-tap-v1` 广播完整调试记录，但不等待调试页也不让广播失败干扰游戏 -> Zod 校验人物连续性、两组三档牌、指定能力和因果账本，缺失可见文案时仅补失败字段 -> 并发生成自然普通话《一生纪事》与 2026 蝴蝶效应报告 -> 两份报告都存在时，分组进度原子地记录首次通关并奖励 1 枚代币；重玩只更新当前局，不重复发币 -> 合并报告并导出完整 2x PNG。
...
- `src/App.tsx`：根组件，负责首次游戏说明、分组入口、游戏 phase 切换、首次通关奖励和结局导出。
...
- `src/screens/SeedPickerScreen.tsx`、`src/components/HistoryGroupCard.tsx` 与 `src/components/GameAnnouncement.tsx`：13 组档案墙、锁定预览、组内选剧本、进度/代币反馈和首次玩法说明入口。
...
- `src/data/historyGroups.ts`：13 个组、区域、时代、顺序和 100 个剧本唯一归属的权威入口。
- `src/services/share.ts`：完整报告图片准备、移动系统分享、桌面下载和 object URL 回收入口。
- `src/services/storage.ts`：版本化当前局存档与旧版永久解锁档案入口。
- `src/services/unlockProgress.ts` 与 `src/hooks/useUnlockProgress.ts`：分组解锁、完成剧本、代币、旧档案迁移、损坏恢复和 React 状态编排入口。
- `src/services/completion.ts`、`src/services/completion-contract.ts`、`src/services/deepseek.ts`、`src/services/seed.interactive.ts` 与 `docs/ai-transports.md`：中性模型调用边界、本地/Sites DeepSeek 通道和互动空间 Seed 通道入口；互动空间入口固定 `reasoning_effort: "high"` 与 `service_tier: "auto"`、直接拼接官方原始文本 SSE，同时兼容旧版 provider SSE 包。
...
### 2026-07-30 04:30 - 完成 13 组渐进解锁、首次通关代币与隔离分支

- 本次任务：依据 `docs/superpowers/specs/2026-07-29-group-unlock-gameplay-design.md`，从本地最新提交创建隔离分支，完整替换随机命运入口为剧本分组、解锁代币和可重复游玩的渐进系统，并保持现有移动端档案卡牌视觉。
- 改了哪些文件：新增 `src/data/historyGroups.ts` 及测试、`src/services/unlockProgress.ts` 及测试、`src/hooks/useUnlockProgress.ts`、`src/components/HistoryGroupCard.tsx`；重构 `SeedPickerScreen` 及测试，接入 `App`、结局页、剧本卡、游戏说明与集成测试，扩展 `game.css`；同步互动空间发布门禁、海报脚本、README、`AGENTS.md` 与本文档。
- 改了什么：100 个历史节点被完整且唯一地分进 13 组（9 组中国史、4 组世界史）；规格未归组的 `sputnik-1957` 与 `web-public-domain-1993` 一并纳入“冷战与当代”。首页展示组别档案、锁定模糊预览、时代、完成数和代币；任意第一组免费，后续组每次花费 1 枚代币。只有两份结局报告都存在并进入结果页时才原子记录首次通关与 `+1` 代币，重玩不重复发币。进度使用独立版本化 localStorage 信封，包含合法值清洗、损坏恢复与旧版档案迁移；退出、重开和当前局存档淘汰都不会清除它。
- 为什么这样改：分组先降低面对 100 个开局的选择负担，再用“一次完整人生换一把新钥匙”形成清晰的长期目标；把奖励提交点放在真实结果页、把进度从当前局中拆开，可避免报告失败、刷新、重玩或旧存档迁移造成重复发币或访问权丢失。
- 影响了哪些模块：首页导航、剧本选择、首次游戏说明、结局奖励、localStorage 迁移、移动端布局、普通 Web 构建和互动空间正式包门禁；四幕、三卡、三次 Roll、AI 请求、世界正史、固定第一幕和双结局内容保持不变。
- 验证：以并发形成的新本地基线 `9d2fba8` 重放后，全量 Vitest 41 文件 387/387、TypeScript、244 个运行时文件可移植性、普通 vinext 构建和 `git diff --check` 全部通过。正式互动空间构建确认 100/100 剧本、100/100 历史图、13 组运行时标记、`reasoningEffort=high`、`serviceTier=auto`、禁止供应商痕迹 0；ZIP 共 140 个文件、5,947,384 字节、MD5 `06719ac2dfe8ed41a55bce81c6d0e742`，官方 `h5-validator` 目录与 ZIP 两轮全部通过。应用内浏览器在首次 IPv6 回环连接失败后被其内部错误页安全策略锁定，未取得可靠的 390×844 视觉截图，因此没有把该项冒充为已通过。

### 2026-07-30 04:23 - Seed 三模型统一最高思考并启用 Fast 自动服务层

- 本次任务：纠正上一版不思考策略，把互动空间 Lite → Pro → Evolving 三个 Seed 模型的全部调用统一改为最高思考强度，并在可用时优先使用火山 Fast 低延迟服务，完成代码、构建、提交和目标草稿覆盖。
- 改了哪些文件：修改 `src/services/seed.interactive.ts` 及其测试、`interactive-space.release.json`、正式包与最小诊断包校验脚本、`diagnostics/interactive-ai-mvp/{index.html,main.js}`、`README.md`、`docs/{ai-transports,interactive-space-release-checklist}.md`、`AGENTS.md` 与本文档。
- 改了什么：正常续幕、第二/第三次 Roll、字段修复、全局恢复、流式空结果的同模型非流式接管和并发双结局全部强制发送 `reasoning_effort: "high"`；Lite、Pro、Evolving 每次请求都同时发送 `service_tier: "auto"`，账户和模型已开通 Fast 时优先使用低延迟层，否则由火山自动回落普通服务，不改变仅额度耗尽才向下一模型接力的稳定性边界。发布契约和两套打包门禁同步锁定 `high` 与 `auto`，互动空间产物仍拒绝任何 DeepSeek 运行时痕迹。
- 为什么这样改：真机反馈表明无思考模式的内容和结构错误率不可接受，产品稳定性优先于单次响应成本；最高思考强度降低生成错误风险，Fast 服务层则在账户能力允许时尽量抵消新增推理延迟，而 `auto` 不会让未开通 Fast 的后备模型直接失败。
- 影响了哪些模块：影响互动空间第 2—4 幕、实时 Roll、全部模型修复/恢复、双结局、最小诊断页、发布契约和构建门禁；不改变本地/Sites DeepSeek 通道、100 剧本、固定第一幕、三卡三 Roll、额度接力条件、存档、视觉或旧审核作品。
- 验证：Seed 定向回归 13/13、全量 Vitest 39 文件 378/378、TypeScript、238 个运行时文件可移植性、普通 vinext 构建、正式互动空间构建、最小诊断包和 `git diff --check` 全部通过。正式包确认 100/100 剧本、100/100 历史图、三个 Seed 精确顺序、`reasoningEffort=high`、`serviceTier=auto`、DeepSeek 运行时痕迹 0；官方 `h5-validator` 对目录和 ZIP 两轮全部通过。正式 ZIP 共 140 个文件、5,939,783 字节，MD5 `092e9ae41e979dee4a883dca8325fc56`。

### 2026-07-30 03:53 - 加入 Seed Lite → Pro → Evolving 额度接力并覆盖互动空间

- 本次任务：在 Seed 2.0 Lite 最速不思考版本上，加入 `doubao-seed-2-0-pro-260215` 与持续更新的 `doubao-seed-evolving`；Lite 额度用完后严格依次接力，同时保持整个互动空间产品不思考、低等待，并覆盖 AppID `tt68891e34f7a3c3e821` 的现有草稿。
- 改了哪些文件：`src/services/seed.interactive.ts` 与测试、`src/services/completion-contract.ts`、`interactive-space.release.json`、正式包校验脚本、README、AI 双通道说明、互动空间发布清单、`AGENTS.md` 与 `PROJECT_CONTEXT.md`。
- 改了什么：正式互动空间模型顺序固定为 Lite → Pro → Evolving，三者的正常生成、字段修复、全局恢复、Roll、双结局和空流接管全部强制 `reasoning_effort: "minimal"`；只有明确的中英文额度/配额耗尽标识才立即切下一模型，普通 429、网络波动与 5xx 留在当前模型短退避重试，鉴权、未开通、参数错误与 `20107` 直接阻断。耗尽模型在当前标签页冷却 30 分钟，后续调用跳过；每次请求指标记录实际 Model ID。发布契约和包内门禁同时锁定三个精确 ID、顺序、接力条件与冷却时间。
- 为什么这样改：额度耗尽是容量池切换，普通限流或瞬时故障则不是；把两者分开可以在 Lite 真正不可用时无等待地用 Pro/Evolving 接管，也避免短时 429 把所有模型一起打穿。`doubao-seed-evolving` 使用无日期 Model ID，后续由火山侧持续升级，产品无需反复改版本号；三个模型都锁定 `minimal`，不会因修复或恢复路径重新开启思考。
- 影响了哪些模块：互动空间全部实时幕次、第二/第三次 Roll、字段修复、全局恢复、并发双结局、错误分类、请求指标、会话级容量记忆、发布契约、ZIP 门禁与平台草稿包；本地/Sites 的 DeepSeek 通道、第一幕固定数据和最小诊断作品不变。
- 验证：Seed 定向回归 13/13、全量 Vitest 39 文件 378/378、TypeScript、238 个运行时文件可移植性、普通 vinext 构建、正式互动空间构建和 `git diff --check` 全部通过。仓库门禁确认 100/100 剧本、100/100 历史图、三个 Seed Model ID 顺序、仅额度耗尽接力、30 分钟冷却、`reasoning_effort=minimal`、DeepSeek 运行时痕迹 0；官方 `h5-validator` 对目录与 ZIP 两轮全部通过。正式 ZIP 共 140 个文件、5,939,769 字节，MD5 `ffa3aac4ab530173dc3d2a56d02c82e4`，SHA-256 `23a22f8b4701b593cff9dec80fbbc63aa6ff58c42c8f8cc28cc119192fdcb1b8`。该 ZIP 已上传为 `tos-cn-i-wfv7lngdqh/2dafca72c50b499eb46f3a5c8f873ce3.zip` 并覆盖 AppID `tt68891e34f7a3c3e821`；平台回读确认 `AIEnabled=true`、竖屏、ZIP、产物说明“Seed三级额度接力版”、MD5 与本地一致，状态仍为 `1`（草稿），本次未直接提审。

### 2026-07-30 03:38 - 合并远端文案并完成 Seed 2.0 Lite 最速不思考适配

- 本次任务：从本地 `zhuanghongkai` 最新版本创建 `codex/seed-lite-minimal`，合并远端 `origin/main` 的最新文案版本，并把互动空间完整产品深度适配为 Doubao Seed 2.0 Lite 的最快不思考模式，随后覆盖 AppID `tt68891e34f7a3c3e821`。
- 改了哪些文件：合并远端 `9c0da452d823` 带来的 `.env.example`、`src/game/engine.ts`、命运首页与牌桌文案/测试、LLM Tap 输出兼容和已知问题记录；新增 `src/services/completion.ts`，将 `deepseek-contract.ts` 改为中性的 `completion-contract.ts`，将 `deepseek.interactive.ts` 及测试改为 `seed.interactive.ts`；修改模型引擎、hooks、生成页、长测、Worker 契约、互动空间 Vite 别名、发布契约、正式包与诊断包门禁、诊断页、README、双通道说明、发布清单、`AGENTS.md` 与本文档。
- 改了什么：远端文案把命运首页改为“尚未揭晓 / 历史就此展开 / 历史流转，即将揭晓”，牌桌说明改为“上划出牌，长按预览”，并补强 Tap 的结局紧凑键处理；互动空间精确使用 `doubao-seed-2-0-lite-260428`，每一次正常续幕、Roll、字段修复、全局恢复、双结局和空流非流式接管都强制发送 `reasoning_effort: "minimal"`，同时保留 4096/2048 token、0.7 温度、SSE 和 90 秒安全窗。正式包的模型入口与共享类型已供应商中性化，构建门禁会拒绝缺失 Seed / `minimal` 标记以及任何 DeepSeek 运行时痕迹；本地与 Sites 的既有 DeepSeek 通道不变。
- 为什么这样改：`minimal` 是 Seed 2.0 Lite 的明确不思考档，能直接消除思考阶段的额外等待；把互动空间供应商实现独立命名并用产物门禁锁死，可以防止未来修复路径、恢复路径或共享类型把高推理和旧供应商痕迹重新带回火山包，同时不破坏本地/Sites 已验证的传输。
- 影响了哪些模块：远端首页与牌桌文案、LLM Tap 兼容、通用模型契约、互动空间 Seed 传输、续幕/Roll/修复/恢复/结局推演、Worker 本地通道、最小诊断页、发布配置、正式 ZIP 门禁、文档与互动空间 AppID `tt68891e34f7a3c3e821`。
- 验证：Seed / 浏览器 / Worker 定向测试 4 文件 51/51、Schema 与双传输定向测试 3 文件 118/118、全量 Vitest 39 文件 373/373、TypeScript、238 个运行时文件可移植性、普通 vinext 构建、最小诊断包、正式互动空间构建和 `git diff --check` 全部通过。仓库门禁确认 100/100 剧本、100/100 历史图、Model ID `doubao-seed-2-0-lite-260428`、`reasoning_effort=minimal`、AI 开启、运行时 DeepSeek 痕迹 0、禁止能力标记 0；官方 `h5-validator` 对目录与 ZIP 的入口、动态代码、网络请求、跳转、外链、内联事件、XSS 和兼容性两轮全部通过。正式 ZIP 共 140 个文件、5,939,210 字节，MD5 `32ac86f3d9ba8f0d403197fa91b96ce6`，SHA-256 `b9e1fa6742b4831f9f3ed04f5e1e0d6f6a5bd35561fe60d193730d1f879a4fb1`。该 ZIP 已上传为 `tos-cn-i-wfv7lngdqh/80996f573efb4661883ca7562bbd4a2f.zip` 并覆盖 AppID `tt68891e34f7a3c3e821`；平台回读确认名称、描述和图标保持不变，`AIEnabled=true`、竖屏、包类型 ZIP、产物说明“Seed最速不思考版”、平台 MD5 与本地一致，状态仍为 `1`（草稿），本次未越过扫码实测直接提审。
...
- 本次任务：按用户纠正后的明确边界，另建一个互动空间作品承载最新 AI 稳定性修复包，不覆盖、不撤审也不删除原 AppID `tt8b1afda71430200121`。
- 改了哪些文件：修改 `interactive-space.release.json` 和本文档；平台新增作品 AppID `ttc88abe3757a063ed21`。
- 改了什么：重新生成 300×300 项目图标，分别上传本地 ZIP 与图标，以创建操作生成全新作品，开启 AI、保持竖屏和正式产品名称；创建后先回读草稿配置，再调用提审并再次回读最终状态。发布契约同步切换到新 AppID，避免后续构建或提交误覆盖旧作品。
- 为什么这样改：用户要求并行保留旧审核版本，新的 AI 传输修复需要一个可独立扫码和审核的目标；新建作品能隔离两版平台状态，并让新包的真机错误类型与空流兜底接受独立验收。
- 影响了哪些模块：只改变互动空间平台作品、当前发布目标和项目状态记录；不改变运行时代码、100 剧本、玩法、模型、prompt/schema、视觉、本地/Sites 产品或旧互动空间作品。
- 验证：新作品回读为 AppID `ttc88abe3757a063ed21`、`AIEnabled=true`、`ScreenDirection=1`、包 MD5 `40c9358edee9b448c8418a3b75cbb62b`，与本地 5,939,194 字节 ZIP 一致；提审接口返回成功，最终状态为 `2`（审核中）。旧 AppID 未调用更新、撤审或删除操作。

### 2026-07-30 02:00 - 修复互动空间空流与平台业务错误误判

- 本次任务：针对“最新扫码测试仍然连接不上 AI”从互动空间真实运行链路重新排查，不再用旧上线包解释最新扫码包的问题；对照平台 AI 接入说明、账号配置、作品配置和火山模型页，修复客户端会延迟并掩盖根因的传输缺陷。
- 改了哪些文件：修改 `src/services/deepseek.interactive.ts`、`src/services/deepseek.interactive.test.ts`、`docs/ai-transports.md`、`AGENTS.md` 和本文档。
- 改了什么：把 HTTP 5xx 判断收紧为 500—599，平台 `40012` 一类五位业务码不再因数值大于 500 而被当作服务故障反复重试；玩家错误保留平台 `errorType`、错误码和原始提示。健康流仍只消费 `onSSE` 正文，平台若结束空流则自动用同一 `tt.callAIChatCompletion` 以 `stream=false` 接管一次并读取完整结果；`success` / `complete` 只在尚无流式正文时接纳 `data`，不会重复拼接，缺失的失败对象和同步启动异常也可安全呈现。
- 为什么这样改：互动空间是独立容器通道，配置存在不代表每次调用一定成功。原实现把平台开发者业务错误误当成可重试的 5xx，既增加十余秒无效等待，又把真正的模型或参数错误压成笼统“连接失败”；单靠流式路径也无法容忍部分手机容器结束空流。修复后优先保证请求能完成，不能完成时必须立即暴露可行动的真实原因。
- 影响了哪些模块：只影响互动空间第 2—4 幕、实时 Roll、字段修复和双结局共用的火山 AI 传输与诊断；不改变本地/Sites 通道、模型 ID、prompt/schema、100 剧本、四幕三 Roll、故事内容、存档、卡牌交互或视觉。
- 验证：只读回查确认作品 `AIEnabled=true`、互动空间账号已有平台托管的火山方舟 Key，火山模型页显示精确 Model ID `deepseek-v4-flash-260425` 且支持 Chat Completions。互动空间定向测试 7/7、全量 Vitest 39 文件 371/371、TypeScript、236 个运行时文件可移植性和 `git diff --check` 通过。新互动空间包包含 100/100 剧本和 100/100 历史图，ZIP 140 个文件、5,939,194 字节、MD5 `40c9358edee9b448c8418a3b75cbb62b`，仓库门禁与官方 `h5-validator` 的目录、ZIP 两轮全部通过。该新包尚未覆盖平台：AppID `tt8b1afda71430200121` 仍处于状态 `2`（审核中），当前 `latest` 仍是上一审核包 MD5 `2456ec6a2ee08c5755c04bddf9e5b97d`，因此不能把本轮本地修复冒充为已在手机扫码实测成功。

### 2026-07-30 01:11 - 按互动空间最新 AI 接入规范修复真实流式调用与显式标识

- 本次任务：逐段阅读比赛飞书文档“新增：如何在作品内接入AI服务”及其引用的最新 AI 服务 API 附录，基于真实代码检查账号/作品开关、模型、调用方式、流式回调、错误语义、客户端版本和 AI 显式标识，并修复会让平台真机生成失败的缺口。
- 改了哪些文件：修改 `src/services/deepseek.interactive.ts` 及测试、`src/screens/SeedPickerScreen.tsx` 及测试、`src/styles/game.css`、`interactive-space.release.json`、`scripts/verify-interactive-review-build.mjs`、`docs/{ai-transports,interactive-space-release-checklist}.md`、`AGENTS.md` 和本文档。
- 改了什么：互动空间传输不再只把 `event.data` 当成 OpenAI JSON 包，而是按官方定义直接拼接原始文本分片，同时保留 `choices[].delta` 旧格式兼容；区分 `open`、`error`、`message`、`[DONE]`，流式成功不再读取可能为空或重复的 `success.data`，`errorType=I` 过程信息不会被错误展示为玩家故障。首页 9px 小标升级为标题栏内 20px 双行 `AI / 生成` 标识，发布契约和构建门禁新增抖音 39.5.0 与画面短边 5% 两项官方要求。
- 为什么这样改：官方接口明确说明 `onSSE(event.data)` 是文本片段；旧实现对非 JSON 分片直接丢弃，平台即使成功返回内容也会被产品判定为空结果。官方同时要求起始画面的 AI 显式标识同时包含“AI/人工智能”和“生成/合成”，且字高不低于画面短边 5%；原 9px 标识在 360—390px 手机上只有要求的一半。
- 影响了哪些模块：影响互动空间第 2—4 幕、实时 Roll、字段修复和双结局共用的火山 AI 流式传输，以及首页起始画面的合规标识和发布门禁；不改变本地/Sites DeepSeek 通道、100 剧本、四幕三 Roll、prompt/schema、故事内容、存档、卡牌交互或报告结构。复核后第 1—3 节已同步更新流式语义、显式标识、最低客户端和关键入口。
- 验证：官方原始文本 SSE、旧版 provider SSE、信息事件、20107、模型 ID 和普通浏览器后备均有定向测试；全量 Vitest 39 文件 369/369、TypeScript、236 个运行时文件可移植性和 `git diff --check` 通过。真实浏览器 360×667 下标识为 20px、44×48px 且完全收在 64px 标题栏内，卡牌与抽取按钮均未被挤出；390×844 抽卡后主次行动完整可见，控制台错误/警告为 0。互动空间包包含 100/100 剧本和 100/100 历史图，ZIP 140 个文件、5,938,998 字节、MD5 `2456ec6a2ee08c5755c04bddf9e5b97d`、SHA-256 `cf9a358d570d637840a30609355fadd9821825ec4180b9d60c184278b435240c`，仓库门禁与官方 `h5-validator` 的目录、ZIP 两轮全部通过。平台 AppID `tt8b1afda71430200121` 已用该 ZIP 覆盖，回读确认竖屏、`AIEnabled=true`、最新包 MD5 与本地一致，提审接口返回成功，最终状态为 `2`（审核中）；审核通过前 `current` 仍保留旧上线包，`latest` 才是本次送审版本。
...
- 验证：定向 3 文件 18/18、全量 Vitest 39 文件 367/367、TypeScript、236 个运行时文件可移植性、vinext 正式构建和 `git diff --check` 通过。390×667 真实页面三张说明均为 `display:block`、三个真实卡框均加载，页面仍恰好 667px 高；上划飞行卡保留完整 `choice-card` 样式、真实框图和说明。4 倍 CPU 降速下抽卡 DOM 从 5 张/210 个后代节点降到 3 张/127 个，约两秒过程的 `TaskDuration` 从修复前 0.630 秒降至 0.378 秒，控制台错误/警告为 0。互动空间包含 100/100 剧本和 100/100 历史图，140 个文件、5,938,740 字节、MD5 `090e51d55acb0346774afb0d97357bd6`，仓库门禁与官方 `h5-validator` 对目录、ZIP 两轮全部通过。平台回读确认 AppID `tt8b1afda71430200121` 的名称、100 节点描述、竖屏、AI 开关与本地 MD5 一致，提审接口返回成功，最终状态为 `2`（审核中）。
