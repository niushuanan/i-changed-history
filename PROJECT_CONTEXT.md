...
- `src/game/`：游戏领域层，包含四维时空人格与能力、12 节点时间计划、结构化 schema、DeepSeek prompts、生成引擎、确定性偏离度和纯 reducer。
...
实际数据流是：四次四选一历史困境 -> 四维人格与时空能力 -> 选择真实历史卡 -> 客户端生成 12 节点目标年份和不重复社会载体 -> 玩家选择 AI 行动或直接写入正史 -> 人格规则、权威结果与因果记忆进入 DeepSeek 结构化 JSON -> 格式归一与 Zod 校验/本地兜底 -> 前端状态机 -> 可见蝴蝶转向 -> 确定性历史偏离度 -> 平行世界 2026 结局 -> PNG 头版。
