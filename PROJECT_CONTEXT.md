...
实际数据流是：选择真实历史卡 -> DeepSeek 生成固定姓名的历史主角 -> 玩家选择 AI 行动或直接写入正史 -> 客户端把姓名、年龄、人生阶段和全部决定固化为不可撤销 `WorldCanon` -> DeepSeek 开放推演同一主角下一次重大冲突 -> 格式归一、Zod、人物连续性与因果账本校验 -> 前端展示“决定、已改变、重大节点、史实分歧” -> 第 12 次决定 -> 主角死亡 -> 一次结局请求同时生成《史记》式白话/文言列传与一页 2026 蝴蝶效应报告 -> PNG 报告。
