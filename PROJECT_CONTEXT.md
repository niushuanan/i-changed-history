...
### 2026-07-30 13:11 - 消除首次 Roll 牌组重复歧义

- 本次任务：用最小提示词改动消除模型把预生成 Roll 牌组复制成首发牌组的歧义。
- 改了哪些文件：修改 `src/game/prompts.ts` 和本文档。
- 改了什么：把续幕输出示例中 `r` 的说明从“与 c 相同”改为“与 c 结构相同、内容不得重复的三项新牌数组”，明确只复用数组结构，不能复用首发三张牌的内容。
- 为什么这样改：旧文案没有区分结构相同与内容相同，模型可能返回两组完全一致的卡牌，导致玩家第一次 Roll 后看见原牌。
- 影响了哪些模块：只影响续幕一次生成中的预生成第一次 Roll 牌组提示；不改变 Schema、三次 Roll 流程、实时第二/第三次 Roll、模型参数、卡牌字段、交互或发布配置。复核后第 1–3 节仍准确，无需调整。
