...
## 3D 历史轮盘与停稳操作分层 QA（2026-07-29）

- 视觉真值：卡片材质、版式与色彩继续以用户选定的 `/var/folders/dt/4fn7m4f50ls_8jkk9vzhsxh80000gn/T/codex-clipboard-8dee6cba-4520-4a0e-97f0-9a68c1d24451.png` 为源；本轮交互基线为修改前真实抽取帧 `/tmp/destiny-carousel-before-390x844.png`，实现帧为 `/tmp/destiny-carousel-final-transition-390x844.png`。两张浏览器截图均为 390 × 844 CSS px、1x 密度、同一“抽取进行中”状态；完整并排输入在 `/tmp/destiny-carousel-comparison-390x844.png`。
- P1 对照历史：旧版每个 tick 会整组重挂载三张同尺寸完整海报，中心卡不发生绕轴转动，三张卡内部还同时显示朱砂 `闯入这一刻`，底部保留大号禁用按钮；运动对象、可执行控件和年份读数同时变化，形成晃眼的平面闪动。新版把下一卡从右后方旋入中心、旧卡转向左后方退场，背景层降亮，整页只保留一个清晰焦点；抽取期间主行动、重抽和禁用按钮均为 0。
- P2 布局修复：360 × 667 下停稳卡宽从旧版 316px 增至 332px，场景和 dossier 合计高度增加约 20px；`闯入这一刻` 与 `换一个开局` 现在属于同一个 `.destiny-revealed` 组，间距约 10px，不再分别被卡片区与视口底部控制。证据为 `/tmp/destiny-carousel-final-reveal2-360x667.png` 和 `/tmp/destiny-carousel-final-reveal2-430x932.png`。
- 运动与舒适度：轮盘只展示九段候选，前段约 110-180ms，后段逐步延长至 650ms，并额外保留 460ms 停稳期；每段通过 `rotateY + translateZ + scale` 连续旋入/退场，不再对整个舞台反复做 opacity/blur 闪变。所有候选本地图在抽取开始时预载并以 eager 渲染；`prefers-reduced-motion` 仍保留同一状态顺序但跳过长距离动画。
- 字体、间距、颜色与图像：书法字标、宋体档案标题、11px 事实字段、煤黑舞台、暖纸 dossier、朱砂年份/主行动和历史本地图全部沿用选定视觉，不引入新字体、颜色、占位图、CSS 插画或替代图标。运动状态不要求玩家阅读正在旋转的副卡；停稳后所有完整文案恢复清晰。
- 响应式与交互：真实浏览器覆盖 360 × 667、390 × 844、430 × 932。三个尺寸均完成 `随机抽一个开局 -> 3D 轮转 -> 停稳 -> 换一个开局`；停稳前 DOM 不存在 `闯入这一刻` / `换一个开局`，停稳后两个按钮均可访问且在首屏。360px 短屏底部仍留安全余量，430px 高屏不再把次级操作沉到底部；应用控制台错误/警告为 0。
- 门禁：全量 Vitest 36 文件 355/355、TypeScript、216 文件可移植性、vinext 生产构建、互动空间三白名单构建和 `git diff --check` 全部通过；审核 ZIP 为 2,713,559 字节，非白名单剧本泄漏为 0。当前无剩余 P0、P1 或 P2；不再为 P3 装饰增加运动，以免重新提高视觉负担。聚焦区域未另做裁切对照，因为完整 390 × 844 并排图已经同时覆盖轮盘运动、卡体纵向比例和底部操作区域。

final result: passed

## 完整历史卡带、游戏说明与三次洗牌 QA（2026-07-29）

- 首页运动对象：抽取过程中不再保留一张静止卡、只让顶部时间线跳点。真实 DOM 同时渲染三张完整 `HistoryCard`，经过 16 个 staged 候选先快后慢横向掠过，最终中心卡、年份读数和随机结果保持同一 seed；设置菜单和抽取页没有再出现可被误认为导航的顶部节点轴。
- 尺寸与首屏：360 × 667 下中心历史档案宽 316px、底部 y=548.8，右侧下一张从 x=283.6 开始形成清晰预览；主抽取按钮底部 y=639，文档 `scrollHeight === innerHeight === 667`。390 × 844 下中心卡宽 344px；430 × 932 下没有横向溢出。短屏只压缩画面高度和装饰间距，没有把卡片缩成小卡片。
- 游戏说明：弹层标题严格为 `游戏说明`，核心句是 `抽一段历史，亲手改写它`；四条说明依次解释随机开局、每幕只出一张牌、每幕三次 Roll、四次选择走完一生与 2026 结局，不再用 `新玩法公告` 或面向开发者的机制术语。390 × 844 下弹层无需内部滚动，主按钮可见。
- 牌桌减负：活动页只保留一条 `上划打出 · 按住读牌 · 每幕可 Roll 3 次`，三张卡内部重复的 `上划选择` / `按住读牌` 和章节口号 `抽一张，改写这一刻` 均为 0。360 × 667 下三牌、紧凑 Roll 和 `正史切入口` 全部在首屏，页面高度严格等于 667px。
- Roll 节奏：第一次预生成 Roll 仍先收牌并等待 1.2 秒，再发出新三张；第二、第三次真实请求先进入 `洗牌中` / `正在为这一刻换一手牌…`，请求错误不会抢在过渡前突然出现。失败保留原牌，点击重试会立即隐藏旧错误并重新收牌；组件测试以假时钟锁定 1.2 秒和重试状态，实机验证了 `is-collecting`、`is-waiting` 与失败回退。
- 文案质量：固定第一幕的循史与破局牌改成具体动宾短语，并额外清理 `并拒绝…` 这类断裂开头；续幕协议要求六张牌先说动作、再说对象和期限，明确禁止 `夺取现场解释权`、`推进既有轨迹`、`改变历史走向`、`重塑秩序`、`综合施策`、`稳妥处置` 等抽象套话。
- 对照检查：`/tmp/history-event-reference-comparison.png` 把用户选定的 360px 等宽牌桌源稿与当前 360 × 667 实机活动页放入同一输入。实现保留煤黑电影表面、历史图、黄铜/朱砂/青绿实体牌、中牌前置、侧牌内倾、紧凑 Roll 和底部历史入口；差异来自四幕产品信息和更短视口，不改变选定稿的材质、层级与核心交互。
- 门禁：真实浏览器覆盖 360 × 667、390 × 844、430 × 932，控制台错误/警告为 0；全量 Vitest 36 文件 355/355、TypeScript、215 文件可移植性、生产构建和互动空间三白名单构建全部通过。审核 ZIP 2,712,598 字节，非白名单剧本泄漏为 0。

final result: passed

## 命运抽取、四决三 Roll 与解锁档案 QA（2026-07-29）

- 视觉边界：用户明确要求保留现有历史档案卡，本轮没有重画海报卡或活动三卡。首页新增的“未显影命运牌”直接复用藏品牌框和档案舞台材质；抽取停稳后仍渲染原 `HistoryCard` 的插画、朱砂竖年、暖纸 dossier 和独立 `闯入这一刻`。`tmp/qa-destiny/reference-vs-current.png` 把用户选定的牌桌真值与 360 × 667 当前活动页放在同一张对照图中，煤黑/新闻纸/朱砂/黄铜/青绿层级、中心牌前置和紧凑 Roll 均保持一致。
- 开局仪式：首次进入的公告用四条玩家任务解释“抽中命运、四次抉择、每幕三次 Roll、完成才解锁”，390 × 844 与 430 × 932 下弹层无需内部滚动，关闭与设置菜单重开都可用。未显影态在 360 × 667 下卡背为 330px 高、主按钮完整落在 y=597..639，文档 `scrollHeight === innerHeight === 667`；390 × 844 下卡背为 417px 高，按钮落在 y=770..816。
- 随机停靠：真实点击 `抽取我的命运` 后，100 节点轴经过 staged 疾驰并只留下一个活动红点；本次停在 `公元前 49 年 · 凯撒渡过卢比孔河`。活动点、年份读数、海报标题三者一致，海报完整信息和 `闯入这一刻` 都在 360 × 667 首屏；再次抽取仍是独立主动作。
- 四幕牌桌：真实进入固定开场后，顶部严格显示 `1/4` 和四节点轨道。完整现场正文、三张实体牌、88px 紧凑 `ROLL · 3` 与 `正史切入口` 同时可见，页面无纵向溢出；并排参考确认只是为更短视口压缩场景和牌桌间距，没有改变既定卡牌解剖。
- Roll：真实第一次点击在原牌收拢后零等待发出预生成第二组三张，按钮变为 `ROLL · 2`；第二次触发现编牌请求。当前本地服务没有可用的模型凭证，真实失败保持当前三牌不丢失，并在 Roll 原位显示 `新牌暂时没有发出来，点击 ROLL 再试。`，不会把整幕推进或伪造卡牌。应用集成测试覆盖第二、第三次请求及 `ROLL · 1` / 用尽状态。
- 解锁：未完整通关时，设置中的 `已解锁档案` 明确显示 `0 / 100` 和“完整走完四次人生抉择”空态；reducer 与存储测试锁定只有双报告完成后才写入 seed ID，退出半局不会误点亮。后续随机算法优先从未解锁池选择并尽量避开上一次落点。
- 响应式与可访问：360 × 667、390 × 844、430 × 932 均实际截图；未显影卡、揭晓卡、公告、四幕牌桌、第一次 Roll、实时 Roll 失败与空档案没有横向溢出。时间轴是只读随机化提示而非伪装成可点击选择器；公告、设置、抽取、进入、Roll、退出和档案均有明确 accessible name。页面控制台错误/警告为 0，`prefers-reduced-motion` 会跳过疾驰与长距离卡牌动画但保留状态变化。
- 自动门禁：Vitest 36 文件 355/355、TypeScript、215 文件可移植性、Sites 生产构建和互动空间三白名单构建全部通过；审核 ZIP 为 2,711,059 字节且非白名单泄漏为 0。三开局真实四幕 soak 已实际发起，但 DeepSeek 官方对本地保存凭证返回 `unauthorized`，因此没有伪称外部模型全局通过；浏览器实时 Roll 的同类失败保留当前牌并提供原位重试，脱敏 soak 证据留在被忽略的 `tmp/soak/destiny-four-turn-20260729/`。

final result: passed

## 藏品级牌面、按住蓄力与短屏牌桌 QA（2026-07-29）

- 视觉真值仍为用户选定的 `/var/folders/dt/4fn7m4f50ls_8jkk9vzhsxh80000gn/T/codex-clipboard-8dee6cba-4520-4a0e-97f0-9a68c1d24451.png`（853 × 1852）。本轮没有更换煤黑、旧纸、朱砂、黄铜与青绿方向，而是修正上一版把实体牌做成多层 CSS 线框、牌面层级细碎、长按只有延时没有蓄力反馈的问题。
- 素材实现：新增三张无文字、无徽标、无交互状态的 512 × 768 原创 WebP 材质框，分别承担循史黄铜、破局朱砂和天外铜绿；所有分类、徽章、短牌名、上划与读牌提示继续是可访问的实时 HTML。中牌前置、侧牌内倾、底沿厚度、磨损金属和压桌阴影现在由一张连续实体表面共同表达，不再靠六层重复边线伪造雕花。
- 布局结果：在更严苛的 390 × 720 产品壳中，完整场景、正文、三张牌、94 × 40px 紧凑 Roll 和正史入口仍同时落在首屏；牌桌得到更大的垂直面积，中牌比侧牌更宽更高，但左右牌的关键文案仍完整可读。`design/validation/collector-card-reference-comparison.png` 把 390px 等宽的源稿与短屏实机放在同一张图中；源稿按比例为 390 × 847，而当前短屏只有 720px，因此只判断共同可见区域的层级、材质、颜色与构件关系，不声称不同高度之间的逐像素一致。
- 按住反馈：指针落下即进入 320ms 蓄力态，来源牌直立、前移并增亮，顶部金属丝从左向右填充，徽章轻微抬升，另外两牌同时后退降亮；超过 8px 位移立即取消蓄力并切回上划手势。完成后详情从同一张牌的材质语言接入，完整展示 canonical 决定、谁来做、怎么做、作用对象、期限、直接结果和隐藏代价，圆形黄铜关闭扣完整可见。证据分别为 `collector-card-hold-390x720.png` 与 `collector-card-detail-390x720.png`。
- 真实交互：in-app browser 中实际点击一次 Roll 后，三张旧牌收拢并换成已经准备好的第二组三张，按钮变为已用；实际快速上划破局牌进入 `这件事已经发生`，没有再现“无法进入下一页”。键盘 Enter 与 320ms 指针计时仍共享同一详情入口，指针移动会取消长按，关闭后焦点返回来源牌。控制台错误/警告为 0。
- 可访问与性能：卡面和详情文字继续是 DOM 文本而不是图片字；详情限制在事件容器内并允许必要滚动，390 × 720 下无需裁切；检查音效只使用两枚短 Web Audio 音色，不引入网络或大音频；`prefers-reduced-motion` 保留蓄力、选中与详情状态，但跳过长距离旅行。
- 回归门禁：组件测试覆盖三张蓄力提示、按住状态、移动取消、详情字段、一次 Roll 和上划提交；事件页测试锁定三张新材质资源。完整验证结果记录在本次 `PROJECT_CONTEXT.md` 交付条目中。

final result: passed

## 紧凑 Roll、可靠出牌与连续转场 QA（2026-07-29）

- P1 根因：卡牌原先在指针移动和抬起时读取 React 的 `dragging` / `armed` state。移动与抬起发生在同一批更新时，`pointermove` 可能仍看到 `dragging=false`，`pointerup` 也可能仍看到 `armed=false`，因此快划会偶发不出牌；`pointercancel` 复用了提交函数，也存在浏览器取消手势时误提交的风险。
- 交互修复：同步 pointer refs 现在是手势真值，位移只在 `requestAnimationFrame` 写入卡牌 CSS 变量；48px 阈值直接读取同步 offset，选中牌在 480ms 内越过顶部，未选牌下沉、降亮，事件场景与正文同步让位，结果页场景聚焦后裁决卡从底部接入。连续两次真实快速坐标上划都进入 `这件事已经发生` 结果页。
- Roll：常规视口为 94 × 40px，720px 及以下短屏为 88 × 36px；旧的 82% / 100% 横向牌匣和三张装饰牌已删除。真实点击先在 180ms 内把三张牌收向紧凑朱砂按钮，再发出预生成第二组三张并显示 `已用`，仍然没有模型等待或第二次可用状态。
- 详情：长按阈值从 430ms 缩短到 320ms；详情根据来源卡片的实际矩形从原位起身并在关闭时回落，去掉模糊滤镜造成的合成压力。关闭控件从普通方形按钮改为 38px 圆形黄铜扣；真实打开/关闭、Escape/键盘同入口和关闭后的源牌焦点恢复都保留。
- 视觉检查：在 390 × 720 产品壳中，三张牌完整可读，紧凑 Roll 与 `正史切入口` 都在首屏内；收牌中、第二组三牌、详情展开、上划飞出和结果页五个状态均通过真实浏览器截图检查。截图仅保存在被忽略的 `tmp/browser-audit/`，不把测试产物写入源码仓库。
- 回归门禁：组件测试新增 React 批处理快划、`pointercancel` 不提交、320ms 长按、详情关闭焦点、180ms Roll 与整页离场态断言；`prefers-reduced-motion` 保留同一状态并缩短时间。全量 Vitest 36 文件 362/362、TypeScript、214 个运行时文件可移植性、vinext 生产构建和 `git diff --check` 通过；互动空间审核包仍为三个白名单剧本且 2,639,815 字节。当前无遗留 P0、P1 或 P2 问题。
